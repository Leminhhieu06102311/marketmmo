...
const PurchaseHistory = () => {
  ...
  const [searchResults, setSearchResults] = useState<Histories[] | undefined>(
    []
  );
  useEffect(() => {
  ...
  const handleSearch = (event: any) => {
    ...
    if (inputValue.length > 0) {
      const results = histories?.filter((userData) => {
        // Handle cases where fields are null
        const name = userData.product.name.toLowerCase(); // Chuyển đổi thành chữ thường
                            ...
                            <div className="flex justify-between border-b border-solid border-[#EFF2F5] py-2">
                              <div className="flex items-center gap-x-4">
                                ...
                                <div>
                                  <button
                                    onClick={() =>
                                      hanldeRedirectRating(item.product.slug)
                                    }
